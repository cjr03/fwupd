...
static gchar *
fu_amd_gpu_uma_setting_read_value(FwupdBiosSetting *setting, GError **error)
{
	FuAmdGpuUmaSetting *self = FU_AMD_GPU_UMA_SETTING(setting);
	g_autofree gchar *carveout_file = NULL;

	carveout_file =
	    g_build_filename(self->uma_path,
			     fwupd_bios_setting_get_filename(FWUPD_BIOS_SETTING(setting)),
			     NULL);
	return fu_amd_gpu_uma_read_file(carveout_file, error);
}

static gboolean
fu_amd_gpu_uma_setting_write_value(FwupdBiosSetting *setting, const gchar *value, GError **error)
{
	FuAmdGpuUmaSetting *self = FU_AMD_GPU_UMA_SETTING(setting);
	g_autofree gchar *carveout_file = NULL;
	g_autoptr(FuIOChannel) io = NULL;

	if (self->uma_path == NULL) {
	...
	carveout_file = g_build_filename(self->uma_path, UMA_CARVEOUT_FILE, NULL);
...
fu_amd_gpu_uma_setting_class_init(FuAmdGpuUmaSettingClass *klass)
{
	...
	bios_setting_class->read_value = fu_amd_gpu_uma_setting_read_value;
}
...
fu_amd_gpu_uma_get_setting(const gchar *device_sysfs_path, GError **error)
{
	...
	fwupd_bios_setting_set_canonical_id(FWUPD_BIOS_SETTING(attr), "video-memory");
	fwupd_bios_setting_add_flag(FWUPD_BIOS_SETTING(attr),
				    FWUPD_BIOS_SETTING_FLAG_USER_FRIENDLY);
	...
	for (guint i = 0; lines[i] != NULL; i++) {
		...
		description = fu_strstrip(parts[1]);
