				     ...
				     GError **error)
{
	...
	if (!fwupd_device_id_is_valid(device_id) &&
	    g_strcmp0(device_id, FWUPD_DEVICE_ID_ANY) != 0) {
		g_set_error_literal(error, FWUPD_ERROR, FWUPD_ERROR_INVALID_DATA, "invalid device ID");
		fu_binder_daemon_method_invocation_return_error(out, *error);
		return STATUS_OK;
	}
